...
void TableControl::fillTitle(const QList<QString> &titles)
{
    if (titles.size() != 2)
    {
        _LOG << "there must be two parameters in the list";

        return;
    }

    ui->lblTitle1->setText(titles.at(0));
    ui->lblTitle2->setText(titles.at(1));
}

void TableControl::fillLine(int line, const QList<QString> &contents)
{
    if (line > 11 || line < 0)
    {
...
void TableControl::fill(QList<QLabel *> &line, const QList<QString> &content)
{
